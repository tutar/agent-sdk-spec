...
如果这些问题不先写清，`TUI`、`Desktop`、`Cloud` 虽然都能复用同一套术语，但程序语义会逐步分叉。
...
- `Cloud` 不是 “TUI runtime + 远程工具”
- `Desktop` 也不应被简化为纯单进程应用，因为它常常包含 UI 进程、runtime 进程、daemon 或 extension host
- 只有当接口在跨进程场景下仍然成立时，三种 hosting profile 才真正共享同一套 SDK 语义
...
- `InProcessBinding`
  适用于 TUI 和一部分嵌入式 desktop runtime
- `LocalIpcBinding`
  适用于 desktop GUI + local daemon / extension host
...
不建议把 TUI/Desktop 和 Cloud 设计成两套平行接口。
...
- TUI/Desktop: direct-call contract
...
- 在 TUI / Desktop 下可以选择 in-process binding
...
三种 hosting profile 的真正区别，应理解为部署拓扑差异，而不只是交互壳差异。

- `TUI`
  通常偏向单机或同进程优化，但不应因此把接口设计成只能同进程使用
- `Desktop`
  常常已经是多进程形态，应优先验证 local IPC 语义
- `Cloud`
...
Shared semantic layer
  + InProcessBinding => TUI / embedded desktop
  + LocalIpcBinding => desktop daemon / extension host
